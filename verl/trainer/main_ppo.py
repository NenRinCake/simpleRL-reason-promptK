...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if valid_response_ids not in response_dict:
                response_dict[valid_response_ids] = 0
            else:
                response_dict[valid_response_ids] +=1
...
def create_prompt(gpu_ids, prompts, prompt_model):
    ...
    for idx, row in new_prompts.iterrows():
        orig_prompt = row['prompt'][0]['content']
...
def run_ppo(config, compute_score=None):
    ...
    if not ray.is_initialized():
        ...
        ray.init(runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 'NCCL_DEBUG': 'WARN', 'CUDA_VISIBLE_DEVICES': '4,5,6,7'}}) # 强化学习时指定的 GPU
...
def main_task(config, compute_score=None):
    ...
    config.trainer.n_gpus_per_node = 4 # 训练时用到的 GPU数
    resource_pool_spec = {
