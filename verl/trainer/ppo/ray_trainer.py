...
class RayPPOTrainer(object):
    ...
    def _create_dataloader_by_k(self, cut_k):
        self.train_dataset = RLHFDataset(parquet_files=self.config.data.train_files,
                                         tokenizer=self.tokenizer,
                                         prompt_key=self.config.data.prompt_key,
                                         max_prompt_length=self.config.data.max_prompt_length,
    ...
    def fit(self):
        ...
        k = self.config.prompt.k
        k_ratio = self.config.prompt.use_k_ratio
        k_decay_step = int(self.total_training_steps * k_ratio) // k
        cut_k = 0
        train_ind = 0 # 记录当前训练数据位置
        temp_ind = 0 # 记录临时训练数据位置

        print("=" * 50 + f"\ttotal training steps: {self.total_training_steps}\t")
        print("=" * 50 + f"\tk: {k}\t")
        print("=" * 50 + f"\tk_ratio: {k_ratio}\t")
        print("=" * 50 + f"\tk_decay_step: {k_decay_step}\t")


        assert not (k > int(self.total_training_steps * k_ratio)), \
            f"k_steps={k} 不应大于 total_training_steps * use_k_ratio={int(self.total_training_steps * k_ratio)}"
        assert not (k_ratio > 1), \
            f"k_ratio 不应大于 1"



        self._create_dataloader_by_k(cut_k=cut_k)
        current_iterator = iter(self.train_dataloader)
        ...
        while self.global_steps <= self.total_training_steps:
            ...
            # 更新 k
            if k == 1:
                if (self.global_steps - 1) == int(k_ratio * self.total_training_steps):
                    k -= 1
                    cut_k += 1
            if (self.global_steps - 1) != 0 and (self.global_steps - 1) % k_decay_step == 0 and k > 1:
                k -= 1
                cut_k += 1
            
            # cut_k 发生变化时更新 DataLoader 和迭代器
            if cut_k != old_cut_k:
                print("=" * 50 + f"\tStep {self.global_steps}: Updating dataset cut_k from {old_cut_k} to {cut_k}\t")
                # 存在训练中整轮数据都使用相同 k 步提示的情况，因此这里直接将全部数据进行处理
                self._create_dataloader_by_k(cut_k=cut_k)
                current_iterator = iter(self.train_dataloader)
                temp_ind = 0
            ...
            try:
                # 暂时使用这种低效的方法😢
                while temp_ind < train_ind:
                    next(current_iterator)
                    temp_ind += 1
                batch_dict = next(current_iterator)
            except StopIteration:
                # 当前 epoch 结束时重新开始
                print("=" * 50 + f"\tStep {self.global_steps}: Epoch finished, restarting DataLoader\t")
                current_iterator = iter(self.train_dataloader)
                train_ind = 0
                temp_ind = 0
                batch_dict = next(current_iterator)

            print("=" * 50 + f"\tStep {self.global_steps} Training Start, k = {k}, cut_k = {cut_k}\t")
            ...
            print("=" * 50 + f"\tStep {self.global_steps} Training End\t")
            self.global_steps += 1
            train_ind += 1
            temp_ind += 1
