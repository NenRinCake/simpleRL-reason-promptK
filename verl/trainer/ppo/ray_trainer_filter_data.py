...
class RayPPOTrainer(object):
    ...
    def fit(self):
        ...
        # =========================== 记录训练集索引 ===============================
        train_index = [sample['index'] for sample in self.train_dataloader.dataset]
        # =========================================================================
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # print(f"--!---- batch_dict_index: {batch_dict['index']}")

                # ============================ 备份并去除采样过的index ============================
                copy_batch_dict = batch_dict
                batch_dict_index = batch_dict['index']
                remove_index(train_index, batch_dict_index)
                # ================================================================================
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        print(f"--!---- 筛选前索引: {batch.non_tensor_batch['index']}")

                        correctness_tensor = batch.batch['correctness']
                        index_tensor = batch.non_tensor_batch['index']
                        rollout_n = self.config.actor_rollout_ref.rollout.n
                        bs = len(correctness_tensor) // rollout_n

                        # 按index分组
                        correct_dict = defaultdict(list)

                        for i in range(len(index_tensor)):
                            idx = index_tensor[i]
                            correct = correctness_tensor[i]
                            correct_dict[idx].append(correct)

                        # 计算每组index的准确率
                        acc_dict = {k: sum(v) / len(v) for k, v in correct_dict.items()}

                        # 筛选阈值内的index
                        low_acc, high_acc = 0, 0.8
                        assert 0 <= low_acc <= 1, "low_acc 必须在 [0, 1] 区间内"
                        assert 0 <= high_acc <= 1, "high_acc 必须在 [0, 1] 区间内"
                        assert high_acc > low_acc, "high_acc 必须大于 low_acc"
                        mid_ids = set(k for k, acc in acc_dict.items() if low_acc <= acc <= high_acc)
                        print(f"--!---- correctness: {correctness_tensor}")
                        print(f"--!---- index: {index_tensor}")
                        print(f"--!---- 按index分组: {correct_dict}")
                        print(f"--!---- 准确率: {acc_dict}")
                        print(f"--!---- 保留的索引: {mid_ids}")

                        # 补充样本

                        needed_num = bs - len(mid_ids)

                        # 需要填充
                        if needed_num > 0:
                            replace_index = get_discard_index(batch_dict_index, mid_ids)
                            sample_index = sample_from_train_index(train_index, needed_num)
                            remove_index(train_index, sample_index)

                            # print(f"--!---- 填充的索引: {sample_index}")
                            # print(f"--!---- batch_dict_index: {batch_dict_index}")
                            # print(f"--!---- replace_index: {replace_index}")

                            for ind, sample_ind in zip(replace_index, sample_index):
                                sample_data = self.train_dataloader.dataset[sample_ind]
                                copy_batch_dict["input_ids"][ind] = sample_data["input_ids"]
                                copy_batch_dict["position_ids"][ind] = sample_data["position_ids"]
                                copy_batch_dict["attention_mask"][ind] = sample_data["attention_mask"]
                                copy_batch_dict["subject"][ind] = sample_data["subject"]
                                copy_batch_dict["level"][ind] = sample_data["level"]
                                copy_batch_dict["unique_id"][ind] = sample_data["unique_id"]
                                copy_batch_dict["data_source"][ind] = sample_data["data_source"]
                                copy_batch_dict["ability"][ind] = sample_data["ability"]
                                copy_batch_dict["reward_model"][ind] = sample_data["reward_model"]
                                copy_batch_dict["extra_info"][ind] = sample_data["extra_info"]
                                copy_batch_dict["index"][ind] = sample_data["index"]

                            # print(f"--!---- new_batch: {copy_batch_dict}")

                            # 重新计算
                            metrics = {}
                            timing_raw = {}
                            batch: DataProto = DataProto.from_single_dict(copy_batch_dict)
                            gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'])
                            with _timer('gen', timing_raw):
                                gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)
                            batch.non_tensor_batch['uid'] = np.array([str(uuid.uuid4()) for _ in range(len(batch.batch))],
                            ...
                            for i, mask in enumerate(sample_mask):
                                if mask:
                                    adjusted_attention_mask[i, -max_len:] = 0
                            metrics.update(compute_response_metrics(batch=batch))
                            if self.config.trainer.remove_clip:
                                batch.batch['attention_mask'] = adjusted_attention_mask
                            truncated_ratio = torch.sum(sample_mask.float()).item()
                            self._balance_batch(batch, metrics=metrics)
                            batch.meta_info['global_token_num'] = torch.sum(batch.batch['attention_mask'], dim=-1).tolist()
                            with _timer('old_log_prob', timing_raw):
                                old_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                        ...
                        print(f"--!---- 筛选后索引: {batch.non_tensor_batch['index']}")
                        # ======================================================================================
...
# 查找去除index的下标
def get_discard_index(batch_dict_index, mid_ids):
    return [i for i, idx in enumerate(batch_dict_index) if idx not in mid_ids]


# 去除训练集的index
def remove_index(train_index, to_remove):
    return [x for x in train_index if x not in to_remove]

# 随机采样
def sample_from_train_index(train_index, n):
    return random.sample(train_index, n)
